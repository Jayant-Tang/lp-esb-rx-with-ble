...
static mpsl_timeslot_signal_return_param_t *ts_extend_succeeded_action(void)
{
	...
	case TS_NEXT_ACTION_RX: {
		 ...
		 */
#if IS_ENABLED(CONFIG_ESB_MPSL_TIMESLOT) && \
	defined(CONFIG_APP_ESB_MPSL_TIMESLOT_RX_WINDOW_US) && \
	(CONFIG_APP_ESB_MPSL_TIMESLOT_RX_WINDOW_US > 0)
		uint16_t _max_ext = (uint16_t)(CONFIG_APP_ESB_MPSL_TIMESLOT_RX_WINDOW_US /
					       ts_duration_params.rx_tx_sequence);
		const uint16_t max_extends = (_max_ext == 0U) ? 1U : _max_ext;
#else
...
static mpsl_timeslot_signal_return_param_t *ts_idle_action(mpsl_timeslot_session_id_t session_id)
{
	...
	case TS_NEXT_ACTION_RX_LAST:
		...
		/* @ref multithreading_lock is acquired by MPSL subsys low_prio_work */
#if IS_ENABLED(CONFIG_ESB_MPSL_TIMESLOT) && \
	defined(CONFIG_APP_ESB_MPSL_TIMESLOT_RX_INTERVAL_US) && \
	(CONFIG_APP_ESB_MPSL_TIMESLOT_RX_INTERVAL_US > 0)
		/* Use NORMAL timeslot for low-power PRX: fixed interval, no continuous scanning. */
		if (ts_next_action == TS_NEXT_ACTION_RX) {
			ts_request_normal.params.normal.distance_us =
				CONFIG_APP_ESB_MPSL_TIMESLOT_RX_INTERVAL_US;
